...
**`webrtc_video_container` 서브 컴포넌트 (카메라 업링크):**

| 노드 이름 | 타입 | 용도 |
|-----------|------|------|
| `cam_render_sync` | DAT Execute | `sensor_table` 변경 감지 → 슬롯별 파이프라인 관리 |
| `layout1` | Layout TOP | 슬롯이 하나라도 연결되면 자동 생성 — 모든 슬롯 컴포지트 |

슬롯별 자동 생성 체인: `web_render_top_{N}` → `transform_top_{N}` (rotate) → `crop_top_{N}` → `video_received_slot{N}` (null TOP) → `layout1`

**`webrtc_video_tx_container` 서브 컴포넌트 (Pro 비디오 다운링크):**

사용자가 베이스 COMP 안에 `video_slot{N}` TOP(임의 TOP)을 직접 배치하면 자동으로 파이프라인이 생성된다:
...
**TX (송신) — 비디오 다운링크 (Pro):**
- 사용자가 베이스 COMP에 `video_slot{N}` TOP을 배치하면 자동 감지
- `webrtc_video_tx_container` 내 슬롯별 파이프라인 자동 생성:
  - `select_video_slot{N}` (In TOP, `../../video_slot{N}` 참조)
  - → `flip_top_{N}` (Flip TOP, flipX — 모바일 화면 방향 보정)
  - → `video_stream_out_{N}` (Video Stream Out TOP, WebRTC 모드)
- `addTrack(conn_id, 'video_out_{slot}', 'video')` 호출 후 createOffer
- `webrtc_reanswer` 수신 후 retry로 `webrtctrack = 'video_out_{slot}'` 자동 선택

**단일 createOffer 흐름 (오디오+비디오 동시):**
1. audio_bus 있으면 → `addTrack(audio)`
2. `video_slot{N}` 있으면 → `addTrack(video)`
...
**TD에서 비디오 다운링크 사용법:**
1. 베이스 COMP 안에 `video_slot{N}` TOP 배치 (임의 TOP — Movie File In, Render TOP 등)
2. 모바일 WebRTC 연결 시 자동으로 `select_video_slot{N}` → `flip_top_{N}` → `video_stream_out_{N}` 생성
3. dev_mode=1 모바일: "TD Stream" 버튼으로 전체화면 확인
4. dev_mode=0 모바일: 풀스크린 배경으로 자동 표시 (touch-pad 뒤, z-index:501)
...
- 모든 연결된 클라이언트에 `config` 메시지 전송 (stale addr는 건너뜀)
