...
│   │   ├── background_chop_exec.py ← w2td_background / w2td_bg_color_bus → bg_color
│   │   ├── flashlight_chop_exec.py ← w2td_flashlight CHOP → flashlight 메시지
│   │   ├── update_execs.py        ← 콜백 DAT들 재연결 유틸리티
│   │   └── w2td_zombie_checker.py ← 주기적으로 좀비 연결 정리
│   ├── py_korail/                 ← Korail 프로젝트 전용 (Script CHOP / GLSL)
│   │   ├── line_chop.py           ← 속도 적분 슬라이딩 윈도우 CHOP (센서 활성 + visibility 체크)
│   │   ├── line_glsl.frag         ← 라인 렌더링 GLSL 프래그먼트
│   │   └── line_glsl_split.frag   ← 분할 라인 렌더링 GLSL 프래그먼트
│   └── sketches/                  ← canvas_runner 스케치 파일 (.js)
│       └── particle.js            ← 파티클 시스템: 오리엔테이션 기반 중력 제거 + 노이즈 터뷸런스
│
...
├── dev-tools/
│   └── load-test/
│       └── index.html             ← 부하 테스트 도구 (싱글 파일 웹앱, 배포 독립)
│
├── development/
│   ├── DEV_DOCS.md                ← 이 파일
│   ├── WEBRTC_PLAN.md
...
| `btn-td-stream` | Fullscreen 진입 버튼. dev_mode=1이면 항상 표시. Videoout=td/js/color이면 활성(클릭 가능), none이면 비활성(opacity 35%, disabled). 라벨: `TD Stream` / `JS Sketch` / `Color View` / `Fullscreen` |
...
**`dev_mode=1`**: 자동 전환 없음. `_updateFullscreenButtonVisibility()` 호출만 하고 리턴. `#btn-td-stream` 버튼이 **항상 오른쪽 상단에 표시**되며, 버튼을 탭해야 fullscreen으로 진입한다.

**`_updateFullscreenButtonVisibility()` 동작:**
- `devMode=false` → 버튼 hidden
- `devMode=true` + `displayMode=td/js/color` → 버튼 활성(클릭 가능), 라벨 변경
- `devMode=true` + `displayMode=none/''` → 버튼 표시되지만 disabled (opacity 35%)
- `applyDevMode()`에서도 호출 → devMode 변경 시 displayMode 이미 설정된 경우에도 즉시 동기화

| 버튼 라벨 | 표시 조건 | 진입 함수 |
|----------|----------|---------|
| "TD Stream" | `displayMode === 'td'` | `enterTdStreamMonitor()` |
| "JS Sketch" | `displayMode === 'js'` | `enterTdStreamMonitor()` |
| "Color View" | `displayMode === 'color'` | `enterTdStreamMonitor()` |
| "Fullscreen" (disabled) | `displayMode === 'none'/''\` | 클릭 시 토스트만 표시 |
...
**`lastBgColor` 캐시:** `onBgColor` 수신 시 displayMode/devMode 관계없이 항상 `localStorage('w2td-last-bg-color')` + 변수에 저장. `color` 모드 진입 시 즉시 적용 (TD에서 새 색이 올 때까지 검은 화면 방지).

**`onBgColor` devMode 게이트:** `displayMode === 'color'` 조건에 더해 `devMode && _devFullscreenMode !== 'color'`이면 body 배경색 적용 차단. devMode에서 Color View 전체화면 진입 전에 메인 UI 배경색이 바뀌는 문제 방지. Color View 진입(`enterTdStreamMonitor → _devFullscreenMode='color'`) 후에는 정상 적용됨.
...
### 19-5-1. Free 버전 `touchdesigner/py/callbacks.py` — Pro 동기화 항목

Free 버전에도 Pro 기능 중 공통 기능을 동기화해 관리함.

**Stale 슬롯 자동 정리** (Pro→Free 포팅):
```python
STALE_TIMEOUT_DEFAULT = 30.0   # Disconnecttimeout config 키로 오버라이드 가능
STALE_CHECK_INTERVAL  = 5.0    # onWebSocketReceiveText에서 rate-limit 적용
```
- `_full_cleanup_slot(webServerDAT, addr, slot)` — sensor_table/touch_table/WebRTC/last_seen 일괄 정리
- `cleanup_stale_slots(webServerDAT)` — 마지막 메시지 후 timeout 초 경과 슬롯 강제 회수
- `onWebSocketReceiveText` 진입 시마다 rate-limit 검사 후 자동 호출

**Haptic 피드백** (Free 버전 websocket.js + app.js):
- `websocket.js`: `onHaptic` 콜백 추가, `msg.type === 'haptic'` 라우팅
- `app.js`: `handleHapticFeedback(data)` 함수 추가 — state 모드(연속 진동)와 pattern 모드 모두 지원
  - state=1: `setInterval(100ms)` 로 `navigator.vibrate(100)` 반복
  - state=0: interval 클리어 + `navigator.vibrate(0)`
  - pattern: `navigator.vibrate([200, 100, 200])` 형태 직접 전달

---

### 19-6. `touchdesigner-pro/py_korail/line_chop.py` — 센서 활성 판별 및 백그라운드 처리

Script CHOP. `sensor_table`에서 연결된 슬롯을 수집해 속도를 시간 적분하여 슬라이딩 윈도우 출력.

**`connected_slots` 수집 로직 (루프 내 필터 순서)**

루프에서 다음 세 가지를 순서대로 체크해 슬롯을 제외한다:

1. `connected != 1` → 미연결 슬롯 제외
2. `visibility == 0` → 백그라운드 슬롯 명시적 제외
3. 센서 다중 축 체크 → 미활성 슬롯 제외
...
for row in range(1, st.numRows):
    if int(st[row, st_h['connected']]) != 1:
        continue
    # 백그라운드(visibility=hidden) 슬롯 명시적 제외
...
**왜 `visibility=0` 체크를 센서 체크보다 앞에 두는가**

`accelerationIncludingGravity.az`는 폰을 평면에 놓아도 항상 ≈ 9.8 m/s² (지구 중력). 센서 브로드캐스트가 멈춰도 마지막 값이 테이블에 남아 있으면 `az > 0.1` 조건이 항상 통과한다. `visibility=0`을 먼저 걸러야 홈 화면으로 나간 폰의 frozen az≈9.8이 타임라인을 계속 진행시키는 문제를 막는다.

**`any_connected` 도출**

이전에는 루프 안에서 `any_connected = True`를 설정해, 센서 비활성 `continue` 전에 이미 플래그가 세워지는 버그가 있었다. 현재는 루프 종료 후 `connected_slots`에서 파생하므로, 실제로 활성 센서 + 포그라운드 상태인 슬롯이 하나라도 있을 때만 `True`가 된다.

**슬롯 유지 정책**

백그라운드 슬롯은 `connected_slots`에서 제외되지만 `_slot_to_key`에는 남아 있다. 포그라운드로 돌아오면 센서 데이터가 재수신되는 즉시 `connected_slots`에 재진입하여 같은 채널에서 선이 자동 재개된다.

---

### 19-7. `touchdesigner-pro/sketches/particle.js` — 오리엔테이션 기반 중력 제거 파티클 스케치
...
### 19-8. `dev-tools/load-test/index.html` — 부하 테스트 도구

가상 모바일 디바이스를 N대 동시 생성해 TD WebSocket 서버에 부하를 가하는 싱글 파일 웹앱. 실제 사용자에게 테스트 도구로 배포 가능.

**기능 요약**

- TD에서 수신한 `config` 메시지 기반으로 `sample_rate`, 활성 센서 항목을 자동 적용
- W2TD 모바일 프로토콜을 그대로 구현: `hello` → `ack` → `client_name` + `screen_info` → sensor 루프 → ping/pong
- Stagger delay: 한꺼번에 접속 스파이크가 생기지 않도록 N ms 간격으로 순차 연결
- 실시간 통계: 연결 수, 패킷/초, 총 패킷, 경과 시간
- 클라이언트 카드 그리드: 슬롯 번호, 연결 상태, 활성 센서 뱃지, 전송 패킷 수
...
open
  → send { type: 'hello' }
  ← recv { type: 'ack', slot: N }
  → send { type: 'client_name', name: 'VirtualDevice-N' }
  → send { type: 'screen_info', width: 390, height: 844, ... }
  ← recv { type: 'config', sample_rate, sensor_motion, sensor_orientation, ... }
  → setInterval(sendSensor, 1000 / sample_rate)
  → setInterval(ping, 5000)
```

**센서 더미 데이터 (SensorGen)**

- `ax/ay`: 랜덤 워크 ±15 m/s², `az`: 중력 포함 (≈9.8) 랜덤 워크
- `ga/gb/gg`: 랜덤 워크 ±360 deg/s
- `oa`: 0~360° 천천히 드리프트, `ob`: ±180°, `og`: ±90°
- `lat/lon`: 서울 부근 고정값 (미세 오프셋)

**배포 방법**

싱글 HTML 파일이므로 의존성 없이 어디서든 호스팅 가능. `wss://` 터널에 접속하므로 HTTPS 페이지에서도 브라우저 차단 없이 동작.
